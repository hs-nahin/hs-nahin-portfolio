...
const projects = [
  ...
  {
    title: 'CareerHive',
    description: 'A comprehensive job portal...',
  ...
  return (
    <section className="py-32 relative overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-20 left-10 w-96 h-96 bg-gradient-to-r from-green-600/10 via-blue-600/10 to-purple-600/10 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-20 right-10 w-80 h-80 bg-gradient-to-r from-purple-600/10 via-pink-600/10 to-blue-600/10 rounded-full blur-3xl animate-pulse" style={{ animationDelay: '2s' }} />
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-16">
          ...
          <p className="text-xl md:text-2xl text-purple-200/80 max-w-4xl mx-auto leading-relaxed">
            Filter and explore my recent work crafted with love and magic.
          </p>
        </div>
        ...
        <div className="flex justify-center gap-4 mb-16 flex-wrap">
          {projects.map((project) => (
            <Button
              key={project.title}
              variant={activeProject === project.title ? 'default' : 'outline'}
          ...
          {activeProject && (
            <Button variant="ghost" onClick={() => setActiveProject(null)} className="text-red-400">
              Show All
            </Button>
          )}
        </div>
        ...
        <div className="projects-grid grid lg:grid-cols-2 gap-16">
          {filteredProjects.map((project) => (
            <Card
              key={project.title}
              className="project-card group overflow-hidden bg-gradient-to-br from-slate-800/40 via-slate-800/20 to-slate-900/40 backdrop-blur-2xl border border-purple-500/20 hover:border-purple-400/40 transition-all duration-700 hover:scale-105 hover:shadow-2xl hover:shadow-purple-500/20 relative"
            >
              {project.featured && (
